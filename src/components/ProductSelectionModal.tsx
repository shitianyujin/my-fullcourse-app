// src/components/ProductSelectionModal.tsx (メーカー絞り込み/表示対応 - 最終版)
"use client";
...
export const ProductSelectionModal: React.FC<ProductSelectionModalProps> = ({ 
  ...
  const [itemRole, setItemRole] = useState(initialRole); 
  ...
  const fetchProducts = useCallback(async (keyword: string, manufacturer: string) => {
    ...
    try {
      ...
      if (response.ok) {
        // APIのレスポンスが常にmanufacturerフィールドを持つことを期待
  ...
  const handleAddItemToCourse = () => {
    ...
    // 💡 親コンポーネントにメーカー情報も渡す (undefined/nullの場合は空文字を渡すことでTypeErrorを防ぐ)
    onProductSelect(
      selectedProduct.id, 
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 p-6">
        <div className="flex justify-between items-center border-b pb-3 mb-4">
          <h2 className="text-xl font-semibold text-gray-800">製品の選択と役割設定</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600">
            <FaTimes size={20} />
          </button>
        </div>

        {/* 💡 検索バーとメーカー絞り込みドロップダウン */}
            ...
            products.map((product) => (
              ...
              >
                ...
                <div>
                  ...
                  {/* 💡 表示時に null/undefined を 'メーカー不明' に置き換え */}
        ...
        {selectedProduct && (
          <div className="p-4 border border-green-300 bg-green-50 rounded-lg mb-4">
            <h3 className="font-semibold mb-2">選択中の製品: {selectedProduct.name} ({(selectedProduct.manufacturer || 'メーカー不明')})</h3>
