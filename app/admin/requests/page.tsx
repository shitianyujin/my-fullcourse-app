...
export default async function AdminRequestsPage({
  ...
  const requests = await prisma.contactSubmission.findMany({
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 className="text-2xl font-bold text-gray-800">商品追加依頼 管理</h2>
            <div className="text-sm text-gray-500 mt-1">
                全 {requests.length} 件を表示中
            </div>
        </div>
        {/* 💡 フィルターコンポーネント配置 */}
        <StatusFilter />
                ...
                requests.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-gray-600 bg-gray-50 p-2 rounded border border-gray-100 whitespace-pre-wrap font-mono leading-relaxed">
                        {item.details}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
